...
<div class="mc_merge_var">
	...
	<input type="text" size="4" maxlength="4" value="" name="'.esc_attr($opt.'[detail2]').'" id="'.esc_attr($opt.'-detail2').'" class="mc_input mc_phone" />
			...
			case 'number':
			default:
			    $placeholder = $var['default'] ? esc_html($var['default']) : esc_html($var['name']);
				$html .= '
	<input type="text" size="18" placeholder="'.$placeholder.'" name="'.esc_attr($opt).'" id="'.esc_attr($opt).'" class="mc_input"/>';
				break;
		}
...
class mailchimpSF_Widget extends WP_Widget {
	...
	function mailchimpSF_Widget() {
		...
		$this->WP_Widget('seatosun_newsletter_widget', __('SeaToSun Newsletter', 'mailchimp_i18n'), $widget_ops);
	}
