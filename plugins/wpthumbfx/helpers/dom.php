<?php
/*
	Class: DomAtomicPressHelper
		Helper class for the DOM extension
*/
class DomAtomicPressHelper extends AtomicPressHelper {
	...
	*/
	public function create($input, $mode = 'html') {
		...
		// create object
		$dom = new AtomicPressDOMDocument();
...
/*
	Class: AtomicPressDOMDocument
		Document Class with extended attributes and functions
*/
class AtomicPressDOMDocument extends DOMDocument {
	...
	public function __construct($version = '1.0', $encoding = 'UTF-8') {
		...
		// set node class
		$this->registerNodeClass('DOMElement', 'AtomicPressDOMElement');
	}
	...
	public function find($query) {
		return $this->xpath()->query(AtomicPressCssSelectorParser::cssToXpath($query, 'descendant::'));
	}
...
/*
	Class: AtomicPressDOMElement
		Element Class with extended attributes and functions
*/
class AtomicPressDOMElement extends DOMElement {
	...
	public function find($query) {
		return $this->query(AtomicPressCssSelectorParser::cssToXpath($query, 'descendant::'));
	}
	...
	public function children($query = null) {
		...
		return $this->query(AtomicPressCssSelectorParser::cssToXpath($query, 'child::'));
	}
	...
	protected function prepareInsert($item) {
		...
		if (is_string($item)) {
			...
			$item = AtomicPressDOMEntities::replaceAllEntities($item);
...
class AtomicPressDOMEntities {
	...
	public static function replaceAllEntities($string) {
		return preg_replace_callback(self::$regex, 'AtomicPressDOMEntities::doReplacement', $string);
	}
...
/*
	Class: AtomicPressCssSelectorParser
