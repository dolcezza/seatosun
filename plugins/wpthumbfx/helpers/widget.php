<?php
/*
	Class: WidgetAtomicPressHelper
		Widget helper class. Create and manage Widgets.
*/
class WidgetAtomicPressHelper extends AtomicPressHelper {

	/* post type */
	const POST_TYPE = 'AtTOMICPRESS';
	...
	*/	
	public function get($id) {
		...
		return is_array($post) ? new AtomicPressWidget($id, $post['post_mime_type'], $post['post_excerpt'], $post['post_title'], $post['post_content'], $post['post_date'], $post['post_modified']) : null;
	}
	...
	*/	
	public function all($type = null) {
		...
		foreach ((array) $this->db->get_results($query) as $post) {
			$widgets[] = new AtomicPressWidget($post->ID, $post->post_mime_type, $post->post_excerpt, $post->post_title, $post->post_content, $post->post_date, $post->post_modified);
		}
...
class AtomicPressWidget {
	...
	*/
	public function __construct($id, $type, $style, $name, $content, $created, $modified) {
		...
		$this->type     = $type;
		$this->name     = $name;
		$this->content  = $atomicpress['data']->create($content);
