<?php
/*
	Class: AssetAtomicPressHelper
		Asset helper class, to manage assets
*/
class AssetAtomicPressHelper extends AtomicPressHelper {
	...
	*/
	public function createString($input, $options = array()) {
		return new AtomicPressStringAsset($input, array_merge($options, $this->options));
	}
	...
	*/
	public function createFile($input, $options = array()) {
		...
		return new AtomicPressFileAsset($url, $path, array_merge($options, $this->options));
	}
	...
	*/
	protected function addAsset($name, $asset) {
		...
		if (!isset($this->assets[$name])) {
			$this->assets[$name] = new AtomicPressAssetCollection();
		}
	...
	*/
	public function cache($file, $asset, $filters = array(), $options = array()) {
		...
		$options = array_merge(array('Gzip' => true), $options);

		// copy gzip file, if not exists
		if ($options['Gzip'] && !$this['path']->path('cache:gzip.php')) {
			@copy($this['path']->path('atomicpress:gzip/gzip.php'), rtrim($this['path']->path('cache:'), '/').'/gzip.php');
		}
...
/*
	Interface:  AtomicPressAssetInterface
		Asset interface
*/
interface AtomicPressAssetInterface {
...
/*
	Class:  AtomicPressAssetOptions
		Asset options class, provides options implementation
*/
abstract class AtomicPressAssetOptions implements ArrayAccess {
...
/*
	Class:  AtomicPressAssetBase
		Asset base class
*/
abstract class AtomicPressAssetBase extends AtomicPressAssetOptions implements AtomicPressAssetInterface {
	...
	*/
    public function getType() {
		return str_replace(array('AtomicPress', 'Asset'), array('', ''), get_class($this));
	}
...
class AtomicPressStringAsset extends AtomicPressAssetBase {
...
class AtomicPressFileAsset extends AtomicPressAssetBase {
...
class AtomicPressAssetCollection extends AtomicPressAssetOptions implements AtomicPressAssetInterface, Iterator {
