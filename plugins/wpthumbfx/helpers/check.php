<?php
/*
	Class: CheckAtomicPressHelper
		System helper class
*/
class CheckAtomicPressHelper extends AtomicPressHelper {
	...
	/*
		Function: notices
			Checks the system for compatability with the atomicpress
	...
	*/
	private function run_tests() {
		...
		if (!file_exists($this['path']->path('cache:')) || !is_writable($this['path']->path('cache:'))) {
			...
			$this->notice_list[] = array(
				'type'    => 'critical',
				'message' => 'AtomicPress cache folder is not writable! Please check directory permissions '.$this['path']->path('cache:')
			);
		}
