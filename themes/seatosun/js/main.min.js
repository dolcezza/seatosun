...
if(SC){var soundCloudPlayer={playerInitialized:false,currentTrackID:null,currentTrackIDList:null,currentSoundObject:null,cachedTrackData:[],cachedSoundObjects:[],firstTrackNumber:0,lastTrackNumber:0,currentTrackNumber:0,playerState:'paused',playerIsMuted:false,currentVolume:100,playerContainer:null,trackTitleElem:null,trackDurationElem:null,playPauseButton:null,previousButton:null,nextButton:null,volumeButton:null,showHidePlaylistButton:null,init:function(playerContainerElement){if(playerContainerElement){this.playerContainer=playerContainerElement;}else{this.playerContainer=$('#soundcloud-player-container');}
this.playerContainer.find('*').off('click.scPlayer');if(this.playerContainer.data('current-track-id-list')){this.currentTrackIDList=this.playerContainer.data('current-track-id-list');}
if(this.playerContainer.data('current-track-id')){this.currentTrackID=this.playerContainer.data('current-track-id');}
if(this.playerContainer.data('current-track-number')){this.currentTrackNumber=this.playerContainer.data('current-track-number');}
this.trackTitleElem=this.playerContainer.find('.track-title');this.trackDurationElem=this.playerContainer.find('.track-duration');this.playPauseButton=this.playerContainer.find('.play-pause');this.previousButton=this.playerContainer.find('.previous');this.nextButton=this.playerContainer.find('.next');this.volumeButton=this.playerContainer.find('.volume');this.showHidePlaylistButton=this.playerContainer.find('.track-list');if(!this.playerContainer.is(':visible')){this.playerContainer.fadeIn();}
this.updateCurrentTrackID();this.updateCurrentTrackInfo();this.previousButton.on('click.scPlayer',function(event){event.preventDefault();soundCloudPlayer.playPreviousTrack();});this.nextButton.on('click.scPlayer',function(event){event.preventDefault();soundCloudPlayer.playNextTrack();});this.playPauseButton.on('click.scPlayer',function(event){event.preventDefault();if(!soundCloudPlayer.playerInitialized){soundCloudPlayer.playerInitialized=true;soundCloudPlayer.playerState='playing';SC.stream('/tracks/'+soundCloudPlayer.currentTrackID,function(sound){soundCloudPlayer.updateCurrentSoundObject(sound);soundCloudPlayer.playCurrentTrack();});}else{switch(soundCloudPlayer.playerState){case'paused':soundCloudPlayer.playCurrentTrack();break;case'playing':soundCloudPlayer.pauseCurrentTrack();break;default:console.log('Invalid player state; only "paused" and "playing" are accepted. Check your code for spelling errors.');break;}}});this.volumeButton.on('click.scPlayer',function(event){event.preventDefault();if(soundCloudPlayer.playerIsMuted){soundCloudPlayer.playerIsMuted=false;soundCloudPlayer.volumeButton.removeClass('muted');soundCloudPlayer.currentSoundObject.unmute();}else{soundCloudPlayer.playerIsMuted=true;soundCloudPlayer.volumeButton.addClass('muted');soundCloudPlayer.currentSoundObject.mute();}});this.showHidePlaylistButton.on('click.scPlayer',function(event){event.preventDefault();soundCloudPlayer.showHidePlaylistButton.toggleClass('visible');});},updateCurrentSoundObject:function(sound){if(!soundCloudPlayer.cachedSoundObjects[soundCloudPlayer.currentTrackID]){soundCloudPlayer.cachedSoundObjects[soundCloudPlayer.currentTrackID]=sound;}
if(soundCloudPlayer.cachedSoundObjects[soundCloudPlayer.currentTrackID]){soundCloudPlayer.currentSoundObject=soundCloudPlayer.cachedSoundObjects[soundCloudPlayer.currentTrackID];}},updateCurrentTrackIDList:function(idList){if(!idList){idList=soundCloudPlayer.playerContainer.data('current-track-id-list');}
if(!idList){idList=soundCloudPlayer.playerContainer.attr('data-track-id-list');}
if(!idList){idList=soundCloudPlayer.playerContainer.find('[data-track-id-list]').attr('data-track-id-list');}
if(idList){if(typeof idList!='object'){try{idList=$.parseJSON(idList);soundCloudPlayer.currentTrackIDList=idList;soundCloudPlayer.firstTrackNumber=0;soundCloudPlayer.lastTrackNumber=$.objectSize(soundCloudPlayer.currentTrackIDList)-1;soundCloudPlayer.currentTrackNumber=soundCloudPlayer.firstTrackNumber;soundCloudPlayer.playerContainer.data('current-track-id-list',idList).data('current-track-number',soundCloudPlayer.currentTrackNumber);}catch(error){console.log(error);}}}},updateCurrentTrackID:function(id){if(!soundCloudPlayer.currentTrackIDList){soundCloudPlayer.updateCurrentTrackIDList();}
if(!id){id=soundCloudPlayer.playerContainer.data('current-track-id');}
if(!id){id=soundCloudPlayer.currentTrackIDList[soundCloudPlayer.currentTrackNumber];}
if(id){soundCloudPlayer.currentTrackID=id;soundCloudPlayer.playerContainer.data('current-track-id',id);}},updateCurrentTrackNumber:function(trackNumber){if(trackNumber!=null){soundCloudPlayer.currentTrackNumber=trackNumber;soundCloudPlayer.playerContainer.data('current-track-number',trackNumber);}},updateCurrentTrackInfo:function(){if(soundCloudPlayer.playerInitialized){soundManager.stopAll();}
...
break;}
if(newTrackID){soundCloudPlayer.updateCurrentTrackNumber(newTrackNumber);soundCloudPlayer.updateCurrentTrackID(newTrackID);soundCloudPlayer.updateCurrentTrackInfo();}},playNextTrack:function(){soundCloudPlayer.playNextOrPreviousTrack('next');},playPreviousTrack:function(){this.playNextOrPreviousTrack('previous');},formatTrackDuration:function(ms){var seconds=Math.floor(ms/1000);var minutes=Math.floor(seconds/60);if(minutes){seconds-=minutes*60;minutes=(minutes<10?'0'+minutes:minutes);}else{minutes='00';}
seconds=(seconds<10?'0'+seconds:seconds);return minutes+":"+seconds;},};var widgetPlayerContainer=$('#soundcloud-player-container');soundCloudPlayer.init(widgetPlayerContainer);var releasesPlayerContainer=$('#releases-soundcloud-player-container');if(releasesPlayerContainer.length){$('.seatosun_release').on('click.scPlayerInit',function(event){event.preventDefault();var self=$(this);var trackIDList=self.attr('data-track-id-list');if(!trackIDList){trackIDList=[self.attr('data-release-id')];}
soundCloudPlayer.updateCurrentTrackIDList(trackIDList);soundCloudPlayer.init(releasesPlayerContainer);});$('#soundcloud-player-container .player-controls .play-pause').on('click.scPlayerInit',function(event){event.preventDefault();var self=$(this);soundCloudPlayer.init(widgetPlayerContainer);});}}});})(jQuery);if(Modernizr){Modernizr.addTest('placeholder',!!("placeholder"in document.createElement("input")));}
